...
CADDrawing::CADDrawing(int timerInterval, QWidget* parent, char* name) :
	QGLWidget(parent) {
...
void CADDrawing::paintGL() {
	...
	glScalef(((float) this->physicalDpiX() / this->curr_width) * zoomlevel,
			((float) this->physicalDpiY() / this->curr_height) * zoomlevel, 1
					* zoomlevel);
	//glRotatef(asin(tan(30)),0.0,0.0f, 1.0f);
	draw_Axes();
	draw_stock();
	draw_scale_marker();
	for (std::list<CADObject*>::iterator It = objects.begin(); It
			!= objects.end(); It++) {
		CADObject* object = *It;
...
void CADDrawing::draw_stock() {
	...
	glVertex3f(stock_xdim - stock_origin_x, 0 - stock_origin_y, 0
			- stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, 0 - stock_origin_y, 0
			- stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, stock_ydim - stock_origin_y, 0
			- stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, stock_ydim - stock_origin_y, 0
			- stock_origin_z);
	glVertex3f(0 - stock_origin_x, stock_ydim - stock_origin_y, 0
			- stock_origin_z);
	glVertex3f(0 - stock_origin_x, stock_ydim - stock_origin_y, 0
			- stock_origin_z);
	glVertex3f(0 - stock_origin_x, 0 - stock_origin_y, 0 - stock_origin_z);
	// Draw Upper Stock Surface
	glVertex3f(0 - stock_origin_x, 0 - stock_origin_y, -stock_zdim
			- stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, 0 - stock_origin_y, -stock_zdim
			- stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, 0 - stock_origin_y, -stock_zdim
			- stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, stock_ydim - stock_origin_y,
			-stock_zdim - stock_origin_z);
	glVertex3f(stock_xdim - stock_origin_x, stock_ydim - stock_origin_y,
			-stock_zdim - stock_origin_z);
	glVertex3f(0 - stock_origin_x, stock_ydim - stock_origin_y, -stock_zdim
			- stock_origin_z);
	glVertex3f(0 - stock_origin_x, stock_ydim - stock_origin_y, -stock_zdim
			- stock_origin_z);
	glVertex3f(0 - stock_origin_x, 0 - stock_origin_y, -stock_zdim
...
void CADDrawing::draw_scale_marker() {
	...
	glBegin(GL_LINES);
	glVertex3d((xscale - 1) - (transX * xscale), (0.5 * yscale) - (1 * yscale)
			- (transY * yscale), 0);
	glVertex3d(xscale - (transX * xscale), (0.5 * yscale) - (1 * yscale)
			- (transY * yscale), 0);
	glEnd();
}
...
bool CADDrawing::add(CADObject *obj, CADObject *prev_obj) {
	...
	if (objects.size() == 0 || prev_obj == NULL) {
		...
		for (LoopIt = objects.begin(); LoopIt != objects.end(); LoopIt++) {
			if (*LoopIt == prev_obj)
				break;
		}
...
void CADDrawing::addLine(double xi, double yi, double zi, double xe, double ye,
		double ze) {

	Line* tmp;

	tmp = new Line(xi, yi, zi, xe, ye, ze);

	add(tmp, NULL);

}

void CADDrawing::addCircle(double x, double y, double z, double r, double i,
		double j, double k) {
	Circle* tmp;

	tmp = new Circle(x, y, z, r, i, j, k);

	add(tmp, NULL);
}

void CADDrawing::changeStock(double x_dim, double y_dim, double z_dim,
...
void CADDrawing::mouseMoveEvent(QMouseEvent *event) {
	...
	gluUnProject(event->x(), event->y(), 0, modelMatrix, projMatrix, viewport,
	...
	emit mouseMoved((position[0] - transX) * (xscale / zoomlevel), (position[1]
			- transY) * (yscale / zoomlevel), position[2]);
